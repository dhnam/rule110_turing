"""From turing machien to tag system
"""
...
def turing_table_to_tag_transition(turing_table: TuringTransitionTable) -> TagSysTransition:
    ...
    for state_k in turing_table:
        ...
        for z in (0, 1):
            if transition[z].move == TuringDirection.LEFT and transition[z].write == TuringSymbol.ONE:
                h_k[z].append(f"R_{transition[z].next_state}*")
                h_k[z].append(f"R_{transition[z].next_state}*")
            if z == 0:
                h_k[z].append(f"H_{transition[z].next_state}")
            h_k[z].append(f"H_{transition[z].next_state}")
            if transition[z].move == TuringDirection.RIGHT and transition[z].write == TuringSymbol.ONE:
...
def turing_machine_to_tag_tape(turing_machien: BinaryTuring) -> TagSysTape:
    """Make tag system tape out of current state of turing machine

    Args:
        turing_machien (BinaryTuring): Source

    Returns:
        TagSysTape: Destination
    """
    tape_state = turing_machien.tape_tuple
    tag_tape_list: list[TagSysSymbol] = [
        f"H_{tape_state.state}:0", f"H_{tape_state.state}:1"]
    for _ in range(tape_state.left_number):
        tag_tape_list += [f"L_{tape_state.state}:0", f"L_{tape_state.state}:1"]
    for _ in range(tape_state.right_number):
        tag_tape_list += [f"R_{tape_state.state}:0", f"R_{tape_state.state}:1"]
...
def tag_tape_to_turing_tape_tuple(tag_tape: TagSysTape) -> TuringTapeTuple:
    ...
    return TuringTapeTuple(
        ...
        head=TuringSymbol.ONE if tape[-2:] == ":1" else TuringSymbol.ZERO,
    )
    

exemple_turing_table = TuringTransitionTable([TuringStateTransition(
    0,
    (
...
exemple_tape_state = TuringTapeTuple(
    state=0,
    left_number=5,
    right_number=21,
...
exemple_turing_machien = BinaryTuring(
    exemple_turing_table, BinaryTuringTape.from_tape_tuple(exemple_tape_state))